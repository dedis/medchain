...
func DoneAction(w http.ResponseWriter, r *http.Request) {
	...
	err = CheckForDone(&request)
	if medChainUtils.CheckError(err, w, r) {
		return
	}
	err = UpdateDatabaseWithDone(&request)
	if medChainUtils.CheckError(err, w, r) {
		return
	}
...
func UpdateDatabaseWithDone(request *signing_messages.ActionUpdate) error {

	err := db_handler.UdpdateActionStatus(request.ActionId, status.Done, db)
	if err != nil {
		return err
	}
...
func CheckForDone(request *signing_messages.ActionUpdate) error {
	...
	if action_status != status.Approved {
		return errors.New("The action needs to be appproved before being commited")
	}
