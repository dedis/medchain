...
func (cl *Client) SpawnNewAdminDarc() (*darc.Darc, error) {
	...
	_, err = cl.bcl.AddTransaction(ctx) //TODO should we use AddTransactionAndWait ?
	if err != nil {
...
func (cl *Client) createAdminDarc() (*darc.Darc, error) {
	...
	adminDarcExpr := expression.InitAndExpr(cl.adminkeys.Identity().String()) // TODO will need to use the method to create threshold multisig rules when implemented
...
func (cl *Client) UpdateAdminRules(evolvedAdminDarc *darc.Darc, newAdminAndExpr, newAdminOrExpr expression.Expr) error {
	...
	err = evolvedAdminDarc.Rules.UpdateRule("spawn:darc", newAdminOrExpr)
	if err != nil {
		return xerrors.Errorf("Updating the invoke:deferred.addProof expression in admin darc: %w", err)
	}
...
func (cl *Client) AddAdminToAdminDarc(adid darc.ID, newAdmin darc.Identity) (byzcoin.InstanceID, error) {
	...
	newAdminAndExpr := expression.InitAndExpr(slc...) //TODO change rules to have multisig with threshold instead
...
func (cl *Client) CreateNewProject(adid darc.ID, pname string) (*darc.Darc, error) {
	pdarcDescription := pname
	rules := darc.InitRules([]darc.Identity{cl.adminkeys.Identity()}, []darc.Identity{cl.adminkeys.Identity()})
	pdarc := darc.NewDarc(rules, []byte(pdarcDescription))
	pdarcActions := "_name:value,spawn:value"
	pdarcExpr := expression.InitAndExpr(cl.adminkeys.Identity().String())
	...
	cl.incrementSignerCounter()
	err = cl.CreateAccessRight(adid, pdarc.GetBaseID())
	if err != nil {
...
// TODO add to util pkg | alternatives to find index of element in a slice
...
// TODO implement AddQuerierToProject()
func (cl *Client) AddQuerierToProject() {

}

// TODO implement RemoveQuerierFromProject()
