...
for n in $( seq $nbr_nodes -1 1 ); do
  ...
  if [ ! -d $co ]; then
    echo -e "$base_ip:$PORT\nMedchain-server_$n\n$co" | $MEDCHAIN_BIN server setup
  fi
  (
    LOG=log/medchain-server_${co}_$PORT
    SHOW=$( [ "$n" -eq 1 -o "$show_all" ] && echo "showing" || echo "" )
    export MEDCHAIN-SERVER_SERVICE_PATH=$(pwd)
    while [[ -f running ]]; do
      echo "Starting medchain-server $LOG"
      if [[ "$SHOW" ]]; then
        $MEDCHAIN_BIN -d $verbose -c $mc/private.toml server 2>&1 | tee $LOG-$(date +%y%m%d-%H%M).log
      else
        $MEDCHAIN_BIN -d $verbose -c $mc/private.toml server > $LOG-$(date +%y%m%d-%H%M).log 2>&1
      fi
      if [[ "$single" ]]; then
